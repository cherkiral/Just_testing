...
def getCSRFToken(guest_token, auth_token, authorization_bearer, proxies):
    ...
    viewer_path, urllib.parse.quote_plus(payload))
...
def login(authorization_bearer, guest_token, username, password, email, proxies):
    ...
    # Flow 3
    data = {"flow_token": flow_token, "subtask_inputs": [{"subtask_id": "LoginEnterUserIdentifierSSOSubtask",
    ...
    if (json.loads(r.text)['subtasks'][0]['subtask_id'] == "LoginEnterAlternateIdentifierSubtask"):
        ...
        data = {"flow_token": flow_token, "subtask_inputs": [
            {"subtask_id": "LoginEnterAlternateIdentifierSubtask", "enter_text": {"text": email, "link": "next_link"}}]}
        user_agent = {'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64; rv:91.0) Gecko/20100101 Firefox/91.0',
                      'Referer': 'https://twitter.com/sw.js', 'X-Guest-Token': guest_token,
                      'Content-Type': 'application/json', 'Authorization': authorization_bearer}
        r = requests.post(url_flow_2, verify=False, headers=user_agent, data=json.dumps(data), proxies=proxies)
